---
title: "Stat331 final project"
output: pdf_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

Setup

```{r}
setwd(r"(C:\Users\Frank Shi\Documents\FrankS\Waterloo\pollutants)")
pollutants <- read.csv("pollutants.csv", header = TRUE)
pollutants <- pollutants[-1]
pollutants
```

```{r}
gender_list <- c('female', 'male')
edu_list <- c('before_high_school', 'high_school', 'college', 'college_grad')
eth_list <- c('other', 'mexi_us', 'nonhisp_black', 'nonhisp_white')
smoke_list <- c('no', 'yes')

gender <- gender_list[pollutants$male + 1]
education <- edu_list[pollutants$edu_cat]
race <- eth_list[pollutants$race_cat]
smoke_now <- smoke_list[pollutants$smokenow + 1]

pollutants$male <- factor(gender)
pollutants$edu_cat <- factor(education)
pollutants$race_cat <- factor(race)
pollutants$smokenow <- factor(smoke_now)

pollutants$male <- factor(gender, levels=gender_list)
pollutants$edu_cat <- factor(education, levels=edu_list)
pollutants$race_cat <- factor(race, levels=eth_list)
pollutants$smokenow <- factor(smoke_now, levels=smoke_list)
```

```{r}
## Factors removed 
pollutants_factorsRemoved <- pollutants[-c(27,28,29,32)]

pollutants_factorsRemovedCor <- cor(pollutants_factorsRemoved)
```

Train and test data

```{r}
set.seed(57)

N <- nrow(pollutants)
sampleTrain <- sample(1:N, round(N*0.8,0), replace = FALSE)

dataTrain <- pollutants[sampleTrain,]
dataTest <- pollutants[-sampleTrain,]
```

Bounds for model selection

```{r}
M0 <- lm(length ~ 1, data = dataTrain)
Mfull <- lm(length ~ ., data = dataTrain)
```

```{r}
## Forward selection using AIC
MfwdAIC <- step(object = M0, scope = list(lower = M0, upper = Mfull), 
             trace = FALSE, direction = "forward", k = 2)
summary(MfwdAIC)

## Prediction accuracy
MfwdAIC.res <- pollutants$length[-sampleTrain] - predict(MfwdAIC, newdata = dataTest)
mspeMfwdAIC <- mean(MfwdAIC.res^2)

print(paste("MSPE of foward selection model based on AIC:",mspeMfwdAIC))
```

```{r}
## Forward selection using BIC
MfwdBIC <- step(object = M0, scope = list(lower = M0, upper = Mfull), 
             trace = FALSE, direction = "forward", k = log(nrow(dataTrain)))
summary(MfwdBIC)

## Prediction accuracy
MfwdBIC.res <- pollutants$length[-sampleTrain] - predict(MfwdBIC, newdata = dataTest)
mspeMfwdBIC <- mean(MfwdBIC.res^2)

print(paste("MSPE of foward selection model based on BIC:",mspeMfwdBIC))
```

```{r}
## Backward selection using AIC
MbckAIC <- step(object = Mfull, scope = list(lower = M0, upper = Mfull),
                trace = FALSE, direction = "backward", k = 2)
summary(MbckAIC)

## Prediction accuracy
MbckAIC.res <- pollutants$length[-sampleTrain] - predict(MbckAIC, newdata = dataTest)
mspeMbckAIC <- mean(MbckAIC.res^2)

print(paste("MSPE of backward selection model based on AIC:",mspeMbckAIC))
```

```{r}
## Backward selection using BIC 
MbckBIC <- step(object = Mfull, scope = list(lower = M0, upper = Mfull),
                trace = FALSE, direction = "backward", k = log(nrow(dataTrain)))
summary(MbckBIC)

## Prediction accuracy
MbckBIC.res <- pollutants$length[-sampleTrain] - predict(MbckBIC, newdata = dataTest)
mspeMbckBIC <- mean(MbckBIC.res^2)

print(paste("MSPE of backward selection model based on BIC:",mspeMbckBIC))
```

```{r}
## Stepwise selection using AIC
M0start <- M0
# aic stepwise
system.time({
  MstepwiseAIC <- step(object = M0start,
                scope = list(lower = M0, upper = Mfull),
                direction = "both",
                trace = 0,
                k = 2)
})
summary(MstepwiseAIC)

## Prediction accuracy
MstepwiseAIC.res <- pollutants$length[-sampleTrain] - predict(MstepwiseAIC, newdata = dataTest)
mspeMstepwiseAIC <- mean(MstepwiseAIC.res^2)

print(paste("MSPE of stepwise selection model based on AIC:",mspeMstepwiseAIC))
```

```{r}
## Stepwise selection using BIC
M0start <- M0
# bic stepwise
system.time({
  MstepwiseBIC <- step(object = M0start,
                scope = list(lower = M0, upper = Mfull),
                direction = "both",
                trace = 0,
                k = log(nrow(dataTrain)))
})
summary(MstepwiseBIC)

## Prediction accuracy
MstepwiseBIC.res <- pollutants$length[-sampleTrain] - predict(MstepwiseBIC, newdata = dataTest)
mspeMstepwiseBIC <- mean(MstepwiseBIC.res^2)

print(paste("MSPE of stepwise selection model based on BIC:",mspeMstepwiseBIC))
```

Correlation testing for PCB 1,2,4,5

```{r}
pollutants_pcbs <- pollutants[,-c(2,3,5,6)]
cor(pollutants_pcbs[,2:8])
```

Stepwise model selection with PCB, Furan, Dioxin, and other correlated features removed

```{r}
pollutants_corRemoved <- pollutants[,-c(2,3,5,6,8,10,13,16,17,21)]
```

```{r}
set.seed(57)

N2 <- nrow(pollutants_corRemoved)
sampleTrain2 <- sample(1:N2, round(N2*0.8,0), replace = FALSE)

dataTrain2 <- pollutants_corRemoved[sampleTrain2,]
dataTest2 <- pollutants_corRemoved[-sampleTrain2,]
```

Bounds for model selection

```{r}
M0_corRemoved <- lm(length ~ 1, data = dataTrain2)
Mfull_corRemoved <- lm(length ~ ., data = dataTrain2)
```

```{r}
## Forward selection using AIC
MfwdAIC_corRemoved <- step(object = M0_corRemoved, 
                           scope = list(lower = M0_corRemoved, upper = Mfull_corRemoved), 
                           trace = FALSE, 
                           direction = "forward", k = 2)
summary(MfwdAIC_corRemoved)

## Prediction accuracy
MfwdAIC_corRemoved.res <- pollutants$length[-sampleTrain2] - predict(MfwdAIC_corRemoved, newdata = dataTest2)
mspeMfwdAIC_corRemoved <- mean(MfwdAIC_corRemoved.res^2)

print(paste("MSPE of foward selection model based on AIC:",mspeMfwdAIC_corRemoved))
```

```{r}
## Forward selection using BIC
MfwdBIC_corRemoved <- step(object = M0_corRemoved, scope = list(lower = M0_corRemoved, upper = Mfull_corRemoved), 
             trace = FALSE, direction = "forward", k = log(nrow(dataTrain2)))
summary(MfwdBIC_corRemoved)

## Prediction accuracy
MfwdBIC_corRemoved.res <- pollutants$length[-sampleTrain2] - predict(MfwdBIC_corRemoved, newdata = dataTest2)
mspeMfwdBIC_corRemoved <- mean(MfwdBIC_corRemoved.res^2)

print(paste("MSPE of foward selection model based on BIC:",mspeMfwdBIC_corRemoved))
```

```{r}
## Backward selection using AIC 
MbckAIC_corRemoved <- step(object = Mfull_corRemoved, scope = list(lower = M0_corRemoved, upper = Mfull_corRemoved),
                trace = FALSE, direction = "backward", k = 2)
summary(MbckAIC_corRemoved)

## Prediction accuracy
MbckAIC_corRemoved.res <- pollutants$length[-sampleTrain2] - predict(MbckAIC_corRemoved, newdata = dataTest2)
mspeMbckAIC_corRemoved <- mean(MbckAIC_corRemoved.res^2)

print(paste("MSPE of foward selection model based on AIC:",mspeMbckAIC_corRemoved))
```

```{r}
## Backward selection using BIC 
MbckBIC_corRemoved <- step(object = Mfull_corRemoved, scope = list(lower = M0_corRemoved, upper = Mfull_corRemoved),
                trace = FALSE, direction = "backward", k = log(nrow(dataTrain2)))
summary(MbckBIC_corRemoved)

## Prediction accuracy
MbckBIC_corRemoved.res <- pollutants$length[-sampleTrain2] - predict(MbckBIC_corRemoved, newdata = dataTest2)
mspeMbckBIC_corRemoved <- mean(MbckBIC_corRemoved.res^2)

print(paste("MSPE of foward selection model based on AIC:",mspeMbckBIC_corRemoved))
```

```{r}
## Stepwise selection using AIC but with reduced set of covariates
M0start_corRemoved <- M0_corRemoved
# aic stepwise
system.time({
  MstepwiseAIC_corRemoved <- step(object = M0start_corRemoved,
                scope = list(lower = M0_corRemoved, upper = Mfull_corRemoved),
                direction = "both",
                trace = 0,
                k = 2)
})
summary(MstepwiseAIC_corRemoved)

## Prediction accuracy
MstepwiseAIC_corRemoved.res <- pollutants$length[-sampleTrain] - predict(MstepwiseAIC_corRemoved, 
                                                                         newdata = dataTest2)
mspeMstepwiseAIC_corRemoved <- mean(MstepwiseAIC_corRemoved.res^2)

print(paste("MSPE of stepwise selection model based on AIC (uncorrelated):",mspeMstepwiseAIC_corRemoved))
```

```{r}
## Stepwise selection using AIC but with reduced set of covariates
M0start_corRemoved <- M0_corRemoved
# aic stepwise
system.time({
  MstepwiseBIC_corRemoved <- step(object = M0start_corRemoved,
                scope = list(lower = M0_corRemoved, upper = Mfull_corRemoved),
                direction = "both",
                trace = 0,
                k = log(nrow(dataTrain2)))
})
summary(MstepwiseBIC_corRemoved)

## Prediction accuracy
MstepwiseBIC_corRemoved.res <- pollutants$length[-sampleTrain] - predict(MstepwiseBIC_corRemoved, 
                                                                         newdata = dataTest2)
mspeMstepwiseBIC_corRemoved <- mean(MstepwiseBIC_corRemoved.res^2)

print(paste("MSPE of stepwise selection model based on AIC (uncorrelated):",mspeMstepwiseBIC_corRemoved))
```

```{r}
print(paste("MSPE of foward selection model based on AIC:",mspeMfwdAIC))
print(paste("MSPE of foward selection model based on BIC:",mspeMfwdBIC))
print(paste("MSPE of backward selection model based on AIC:",mspeMbckAIC))
print(paste("MSPE of backward selection model based on BIC:",mspeMbckBIC))
print(paste("MSPE of stepwise selection model based on AIC:",mspeMstepwiseAIC))
print(paste("MSPE of stepwise selection model based on BIC:",mspeMstepwiseBIC))

print(paste("MSPE of foward selection model based on AIC (no corr):",mspeMfwdAIC_corRemoved))
print(paste("MSPE of foward selection model based on BIC (no corr):",mspeMfwdBIC_corRemoved))
print(paste("MSPE of backward selection model based on AIC (no corr):",mspeMbckAIC_corRemoved))
print(paste("MSPE of backward selection model based on BIC (no corr):",mspeMbckBIC_corRemoved))
print(paste("MSPE of stepwise selection model based on AIC (no corr):",mspeMstepwiseAIC_corRemoved))
print(paste("MSPE of stepwise selection model based on BIC (no corr):",mspeMstepwiseBIC_corRemoved))
```
Remarks
1. Removing the correlated covariates does not change the MSPE for any model selection algorithm except
for a very small change in the MSPE for forward selection based on AIC.
2. Forward, backwards, and stepwise selection based on BIC all give the same model: furan3 + ageyrs
3. Forward and backwards selection based on AIC give different models.
4. Stepwise based on AIC gives the same model as forward selection based on AIC.




















