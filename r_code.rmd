...
for (i in 1:11) {
  ...
  pollutants[i_colname] <- (pollutants[[i_colname]] - 
                              mean(pollutants[[i_colname]])) / sd(pollutants[[i_colname]])
}
for (i in 1:3) {
...
kfold <- 11  
# 11 because Ntrain is divisible by 11, and 11 is close to the convention of 10-fold CV
...
selection_mat <- matrix(0, nrow=length(colnames(dataTrain)) + 6, ncol=kfold)  
# +6 to rownum bc of categorical covariates
pval_mat <- matrix(0, nrow=length(colnames(dataTrain)) + 6, ncol=kfold)
mat_rownames <- c(colnames(dataTrain), 
                  c("edu_cathigh_school", "edu_catcollege", 
                    "edu_catcollege_grad", "race_catmaxi_us", 
                    "race_catnonhisp_black", "race_catnonhisp_white"))
...
for (i in 1:kfold) {
  ...
  # indicator of which covariates are selected
  selection_mat[mat_rownames %in% row.names(i_coef_df), i] <- 1
  # store the p-values of selected covariates in the matrix
...
covariate_set3 <- c('ageyrs', 'POP_furan3', 'ln_lbxcot', 
                    'yrssmoke', 'POP_PCB1', 'POP_PCB5', 'edu_cat')
...
model3 <- lm(length ~ ageyrs + POP_furan3 + ln_lbxcot + 
               edu_cat + yrssmoke + yrssmoke * ln_lbxcot, 
             data=pollutants_set3)
summary(model3)

# proto_model3 shows that ln_lbxcot on its own has no statistical significance
proto_model3 <- lm(length ~ ageyrs + POP_furan3 + 
                     ln_lbxcot + edu_cat, data=pollutants_set3)
...
studlin <- reslin/(sigma(model3)*sqrt(1-hatvalues(model3))) 
...
dim(DFBETAS)
## beta1 (furan3)
plot(DFBETAS[, 2], type="h", 
     xlab="Obs. Number", ylab=expression(paste("DFBETAS: ",beta[1])))
...
## beta2 (gender)
plot(DFBETAS[,3], type="h",
     xlab="Obs. Number", ylab=expression(paste("DFBETAS: ",beta[2])))
...
## beta3 (age)
plot(DFBETAS[, 4], type="h",
     xlab="Obs. Number", ylab=expression(paste("DFBETAS: ", beta[3])))
...
summary(lm(POP_PCB1 ~ POP_PCB5 + POP_furan3 + edu_cat + 
             ln_lbxcot + ageyrs, data=pollutants_original))
summary(lm(POP_PCB5 ~ POP_PCB1 + POP_furan3 + edu_cat + 
             ln_lbxcot + ageyrs, data=pollutants_original))
